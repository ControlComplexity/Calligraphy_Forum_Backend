...
import (
	...
	"calligraphy-forum/pkg/uploader"
...
func Generate() {
	...
	sm.Add(stm.URL{
		{"loc", forumurls.AbsUrl("/")},
		{"lastmod", time.Now()},
		{"changefreq", changefreqHourly},
	})

	sm.Add(stm.URL{
		{"loc", forumurls.AbsUrl("/topics")},
		{"lastmod", time.Now()},
		{"changefreq", changefreqHourly},
	})

	sm.Add(stm.URL{
		{"loc", forumurls.AbsUrl("/articles")},
		{"lastmod", time.Now()},
	...
	services.ArticleService.ScanDesc(func(articles []model.Article) {
		for _, article := range articles {
			if article.Status == constants.StatusOk {
				articleUrl := forumurls.ArticleUrl(article.Id)
				sm.Add(stm.URL{
					{"loc", articleUrl},
	...
	services.TopicService.ScanDesc(func(topics []model.Topic) {
		for _, topic := range topics {
			if topic.Status == constants.StatusOk {
				topicUrl := forumurls.TopicUrl(topic.Id)
				sm.Add(stm.URL{
					{"loc", topicUrl},
	...
	services.TagService.Scan(func(tags []model.Tag) {
		for _, tag := range tags {
			tagUrl := forumurls.TagArticlesUrl(tag.Id)
			sm.Add(stm.URL{
				{"loc", tagUrl},
				{"lastmod", time.Now()},
				{"changefreq", changefreqMonthly},
